...
void write_file(char* fp1, char* fp2){
    ...
    op_file = open(fp2,O_WRONLY|O_CREAT|O_TRUNC,S_IRUSR|S_IWUSR);
...
int main(int argc, char *argv[]) 
{
    ...
    while ((opt = getopt(argc, argv, "rRib")) != -1) {
        ...
        case 'b':
            ...
            break;
        default:
            fprintf(stderr, "mycp : mycp [-i] [파일1 파일2 ...]\n");
                exit(EXIT_FAILURE);
        //예외처리
            break;
        }
    ...
    //printf("optind : %d argc: %d optind_i:%d \n",optind,argc,option_i);
    ...
    else if(argc-optind > 2){//여러 파일들이 들어왔을 때 가장 마지막이 디렉토리인지 검사
    ...
    for (int i = optind; i < argc-1;i++){ // 마지막 파일을 제외하고 존재 여부 확인
        if(stat(argv[i],&statbuf1) < 0){
    ...
    if (argc - optind == 2 && S_ISREG (statbuf1.st_mode)){
        if(!access(argv[optind+1],W_OK) && option_i){     // 덮어쓰는지 확인 하는 옵션  
